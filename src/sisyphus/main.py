...
@click.option("-l", "--log-level", type=click.Choice(["error", "warning", "info", "debug"], case_sensitive=False),
              default="info", show_default=True, help="Logging level.")
def auto(host, package, branch, destination, linux, windows, instance_type, lifetime, token, log_level):
    """
    ...
    if build_success:
        ...
        if host:
            logging.warning("A host was specified manually, so we won't stop it automatically")
        else:
